          ...
          <div class="card-header text-white bg-primary">
            ...
            <span class="float-right dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton">
                    <a class="dropdown-item" href="{{ route('manager.manage') }}"><i class="fa fa-gear"></i> Manage Employees</a>                                
                </div>
            </span>
          </div>
          <div class="card-body">
                  ...
                  <div class="row padding">
                    ...
                    <div class="col-md-2 col-4">
                      ...
                      @foreach(auth()->user()->shifts as $shift)
                        ...
                        @endphp
                          <div class="form-group">
                              {{ Helper::getStartToEndAMPM($shift->start, $shift->end) }}
                          </div>
                      @endforeach
                    </div>
                    @php
                    ...
                    @foreach(auth()->user()->employees->sortBy('position_id') as $employee)
                      ...
                      @php
                        ...
                        $count = auth()->user()->employees->where('position_id', $employee->position->id)->where('active',1)->count();
                      @endphp
                      <div class="col-md-2 col-4">
...
<div class="modal fade" id="modal" tabindex="-1" role="dialog" aria-labelledby="modal" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <form action="{{ url('manager/dashboard/setting/shift/create') }}" method="POST">
        @csrf
...
<script>
  $(document).ready(function(){
    ...
    $('.settings').on('change', function(){
      if($(this).is(':checked')){
          var url = "{{ url('manager/dashboard/setting/update/') }}";
          $.ajax({
              url: url,
              type: 'POST',
              data: {
    ...
    $('input[type="time"]').on('change', function(){
        var url = "{{ url('manager/dashboard/setting/shift/update') }}";
        $.ajax({
            url: url,
            type: 'POST',
            data: {
    ...
    $('.shift_active').on('change', function(){
      if($(this).is(':checked')){
          var url = "{{ url('manager/dashboard/setting/shift/activate') }}";
          $.ajax({
              url: url,
              type: 'POST',
              data: {
    ...
    $('.criteria').on('change', function(){
      if($(this).is(':checked')){
          var url = "{{ url('manager/dashboard/setting/criteria/update') }}";
          $.ajax({
              url: url,
              type: 'POST',
              data: {
    ...
    $('.delete_shift').on('click', function(){
      var url = "{{ url('manager/dashboard/setting/shift/delete') }}";
        $.ajax({
            url: url,
            type: 'POST',
            data: {
    ...
    $('.minimum').on('change', function(){
      ...
      var url = "{{ url('manager/dashboard/setting/shift/required/create') }}";
      $.ajax({
          url: url,
          type: 'POST',
          data: {
    ...
    $('.btnDays').on('click', function(){
        ...
        var url = "{{ url('manager/dashboard/setting/schedule-dayoff/update') }}";
        $.ajax({
            url: url,
            type: 'POST',
            data: {
